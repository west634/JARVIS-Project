...
import { getStudentWeeklySchedule } from "@/lib/services/schedule";
...
export default async function StudentSchedulePage() {
  ...
  const entries = await getStudentWeeklySchedule(session.schoolId, session.userId);
  ...
  const cancelledToday = entries.filter((e) => e.dayOfWeek === now.getDay() && e.isCancelledToday);

  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Schedule</h1>
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Your weekly class timetable.</p>
      </div>
      ...
      {cancelledToday.length > 0 && (
        <Card className="border-red-200 dark:border-red-500/30">
          <CardHeader title="Today's changes" />
          <ul className="flex flex-col gap-1.5">
            {cancelledToday.map((e) => (
              <li key={e.scheduleBlockId} className="text-sm font-medium text-red-600 dark:text-red-400">
                {e.courseName} cancelled today{e.substituteName ? ` — substitute: ${e.substituteName}` : ""}
              </li>
            ))}
          </ul>
        </Card>
